...
// Initializes TIM2 for microsecond delay (32MHz clock -> prescaler 31 for 1us tick)
void Delay_Init(void)
{
	UTIL_TIMER.Instance = UTIL_TIMER.Instance;
	UTIL_TIMER.Init.Prescaler = 31; // 32MHz/32 = 1MHz (1us per tick)
	UTIL_TIMER.Init.CounterMode = TIM_COUNTERMODE_UP;
	UTIL_TIMER.Init.Period = 0xFFFF;
	UTIL_TIMER.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
	UTIL_TIMER.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_DISABLE;
	if (HAL_TIM_Base_Init(&UTIL_TIMER) != HAL_OK)
	{
		Error_Handler();
	}
	HAL_TIM_Base_Start(&UTIL_TIMER);
}

// No DWT on Cortex-M0+, use timer for microsecond delay
// Assumes UTIL_TIMER (htim2) is initialized, running at 32MHz, 1us per tick
void delay_us(uint32_t us)
{
	uint32_t start = __HAL_TIM_GET_COUNTER(&UTIL_TIMER);
	while (((__HAL_TIM_GET_COUNTER(&UTIL_TIMER) - start) & 0xFFFF) < us);
}
